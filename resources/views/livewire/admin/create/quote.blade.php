<div x-data="{
    ...
    updatePrice(product) {
        if (!product.price_type || product.price_type === 'GENERAL') {
            product.price = Number(product.price_a);
        } else if (!product.price_type || product.price_type === 'A1') {
    ...
    <form wire:submit='save' class="space-y-4">
        ...
        <div class="lg:flex lg:gap-4">
            <x-forms.select label="Producto"
                placeholder="{{ blank($warehouse_uuid) ? 'Debe seleccionar un almacen' : 'Buscar productos...' }}"
                :async-data="[
                    'api' => route('admin.products_warehouses'),
                    'method' => 'POST',
                    'params' => ['limit' => 10, 'warehouse_uuid' => $warehouse_uuid],
                ]" option-label="name" option-value="uuid" wire:model="product_uuid" :disabled="blank($warehouse_uuid)" />
            <div class="">
        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center gap-2">
                ...
                <x-button type="submit" class="mt-4" spinner="save" wire:target="save"
                    wire:loading.attr="disabled" :disabled="count($errors) > 0 || count($products) === 0">
                    Crear Cotización
                </x-button>
            </div>
