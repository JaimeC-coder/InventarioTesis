...
class GraficaSegunda extends Component
{
    ...
    private function cargarGrafica($year): void
    {
        ...
        $labels = collect(range(1, 12))
            ->map(fn($month) => Carbon::create($year, $month, 1)->translatedFormat('F'))
            ->toArray();

        $ventas = collect(range(1, 12))
            ->map(fn($month) => (float) ($ventasPorMes->get($month)->total ?? 0))
            ->values();

        $compras = collect(range(1, 12))
            ->map(fn($month) => (float) ($comprasPorMes->get($month)->total ?? 0))
            ->values();
